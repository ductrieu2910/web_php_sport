...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        ...
        // 2. Lấy được ra các dữ liệu từ trang quan_tri_don_hang.php chuyển sang
        $don_hang_id = $_GET['id'];

        // 3. Viết câu lệnh sql để xóa dữ liệu vào trong bảng hoadon ở trong CSDL
        $sql = "
               DELETE 
               FROM `hoadon` 
               WHERE `hoadon`.`mahoadon` = '".$don_hang_id."'
               ";

        // 3. Thực thi câu lệnh truy vấn (mục đích là để trả về dữ liệu các bạn muốn lấy)
        $xoa_don_hang = mysqli_query($ket_noi, $sql);

        // 5. Hiển thị ra thông báo bạn đã xóa tin tức thành công và đẩy về trang quan_tri_don_hang.php
        ...
        echo "
             <script type='text/javascript'>
                  window.location.href='quan_tri_don_hang.php';
             </script>";    
