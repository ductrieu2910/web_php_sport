...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        ...
        // 2. Lấy được ra các dữ liệu từ trang quan_tri_thanh-vien.php chuyển sang
        $thanhvien_id = $_GET['id'];

        // 3. Viết câu lệnh sql để xóa dữ liệu vào trong bảng thanhvien ở trong CSDL
        $sql = "
               DELETE 
               FROM `thanhvien` 
               WHERE `thanhvien`.`mathanhvien` = '".$thanhvien_id."'
               ";

        // 3. Thực thi câu lệnh truy vấn (mục đích là để trả về dữ liệu các bạn muốn lấy)
        $xoa_thanh_vien = mysqli_query($ket_noi, $sql);

        // 5. Hiển thị ra thông báo bạn đã xóa tin tức thành công và đẩy về trang quan_tri_thanh_vien.php
